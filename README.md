...
```
  ...
  mini reachable as:  192.168.50.176 (LAN)  |  mac-mini.tailde4b5e.ts.net (tailnet)
```
...
`mac-mini.tailde4b5e.ts.net` (its `100.x` IP can change, so prefer the name),
...
3. Point `.env` at the tailnet name instead of the LAN IP:
   ...
   ```ini
   DATABASE_URL=postgresql+psycopg://alex:<password>@mac-mini.tailde4b5e.ts.net:55432/alex
   LOCAL_BASE_URL=http://mac-mini.tailde4b5e.ts.net:11434/v1
   ```
...
```bash
curl http://mac-mini.tailde4b5e.ts.net:11434/api/create -d '{
...
for m in gemma4-12b-ctx16k:latest qwen3-vl-ctx16k:latest; do
  curl -s http://mac-mini.tailde4b5e.ts.net:11434/api/generate \
    -d "{\"model\":\"$m\",\"prompt\":\"\",\"keep_alive\":0}" > /dev/null
done
curl -s http://mac-mini.tailde4b5e.ts.net:11434/api/ps   # confirm nothing is loaded
```
